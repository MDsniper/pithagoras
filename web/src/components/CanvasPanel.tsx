...
export function CanvasPanel({sessionId,open,setOpen}:{sessionId:string;open:boolean;setOpen:(open:boolean)=>void}) {
  ...
  return <div className={`session-canvases ${open?'is-open':''}`}>
    <button className="canvas-toggle" onClick={()=>setOpen(!open)} aria-expanded={open} aria-label="Session canvases"><LuFileText/> Canvases {rows.length>0&&<span>{rows.length}</span>}</button>
